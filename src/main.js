...
ipcMain.handle('export-report-to-excel', async (event, reportData, month, year) => {
    try {
        ...
        if (filePath) {
            ...
            header.push('Total Ore', 'Zile Lucrate', 'Zile Libere', 'Delegație', 'Zile CFP', 'CM', 'Nemotivate', 'Zile CO', 'Zile IP', 'Lichidare');
            ...
            reportData.employees.sort((a, b) => {
                ...
                for (let day = 1; day <= daysInMonth; day++) {
                    ...
                    if (record) {
                        switch(record.status) {
                            ...
                            case 'liquidation': cellContent = 'LCH'; liquidationDays++; cellStyle.fill = { fgColor: { rgb: "F4CCCC" } }; break;
                        }
                    }
                ...
                ws[XLSX.utils.encode_cell({c: daysInMonth + 11, r: row})] = { v: liquidationDays, t: 'n', s: totalStyle };
            });
            ...
            const summaryColumns = ['Total Ore', 'Zile Lucrate', 'Zile Libere', 'Delegație', 'Zile CFP', 'CM', 'Nemotivate', 'Zile CO', 'Zile IP', 'Lichidare'];
